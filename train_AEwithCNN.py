...
def main():
    ...
    if args.evaluate:
        print("=> loading checkpoint ")
        checkpoint2 = torch.load('./checkpoints/AutoEncoderCNN/model_best.pth.tar')
        modelCNN.load_state_dict(checkpoint2['AECNN_state_dict'], strict=False)
    ...
    # evaluate(test_loader, model, modelCNN, criterion)
        # return
    optimizer = torch.optim.SGD(modelCNN.parameters(), args.lr, momentum=args.momentum,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print('\n*** Start Training *** \n')
        # _, train_acc1, _ = train(train_loader, model, modcriterion, optimizer, epoch)
        loss_train,train_acc1= train(train_loader, model, modelCNN,criterion, optimizer, epoch)
        # loss_epochs.append(loss_train)
        train_accs.append(train_acc1.data)
        print('\n*** Start Testing *** \n')
        test_loss, test_prec1, _ = evaluate(test_loader, model, modelCNN, criterion)
        ...
        for tag, value in info.items():
            ...
            save_checkpoint({
                'epoch': epoch + 1,
                'AECNN_state_dict': modelCNN.state_dict(),
...
def train(train_loader, model, modelCNN,criterion, optimizer, epoch):
    batch_time = AverageMeter()
