...
def main():
    ...
    if args.net == "cnn":
        ...
        model_rgb = models.RGBClassifier()
        model = torch.nn.DataParallel(model_rgb).cuda()
    elif args.net == "intensity":
        model = models.IntensityClassifier()
        model = torch.nn.DataParallel(model).cuda()
    elif args.net == "sift":
        model = models.SIFTClassifier()
        model = torch.nn.DataParallel(model).cuda()
    elif args.net =="autoencoder":
        model1 = models.AutoEncoder()
        model1 = torch.nn.DataParallel(model1).cuda()
        model2 = models.EncodeCNN()
    ...
    if args.test != '':
        if os.path.isfile(args.test):
            ...
            print("=> loading checkpoint for test'{}'".format(args.test))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        print('\n*** Start Training *** \n')
        _,train_acc1,_=train(train_loader, model, criterion, optimizer, epoch)
        train_accs.append(train_acc1.data)
        # evaluate on validation set
        print('\n*** Start Testing *** \n')
        test_loss, test_prec1, _ = evaluate(test_loader, model, criterion)
        test_accs.append(test_prec1.data)
        info = {
    ...
    generate_plots(range(args.start_epoch, args.epochs),train_accs,test_accs,args.net,args.loss)
...
def svm_loss(x, y):
    ...
    margins = x - correct_class_scores[:, None] + 1.0
...
def generate_plots(epochs, train_acc, test_acc, model,loss):
    plt.plot(epochs, train_acc, label='train', color='r')
    plt.plot(epochs, test_acc, label='test', color='b')
    plt.title( model+ " "+loss+" accuracy")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.savefig("./plotss/" + model +"_"+loss+"_accuracy.png")
